...
| SEO | Unique titles/descriptions, canonicals, Open Graph, Twitter cards, JSON-LD (LocalBusiness, FAQPage, BreadcrumbList), sitemap.xml, robots.txt, breadcrumbs |
...
- Per-page `$pageTitle` / `$pageDescription` / `$pageCanonical` set in each template.
- JSON-LD: LocalBusiness (homepage), FAQPage (every FAQ section), BreadcrumbList (inner pages).
- `/sitemap.xml` is generated by `sitemap.php`; `/robots.txt` blocks admin, actions, includes, config and uploads.
...
- [x] SEO metadata, JSON-LD, sitemap.xml, robots.txt at the required URLs
...
- [ ] **You**: replace placeholder business values in `config/site.php`, add real customer reviews in
      `/admin/testimonials`, upload real photos, and set production `.env` values.
